...
////////////////////////////////////////////////////////////////////////////////
{
    ...
    struct uidlist list1[] =
    {
		...
		{ { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF }, "Session Manager" },
		{ { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x01 }, "Session Properties Method" },
		{ { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x02 }, "StartSession Method" },
		{ { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x03 }, "SyncSession Method" },
		{ { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x06 }, "CloseSession Method" },
		...
		{ { 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x08, 0x01 }, "Reactivate" },
		{ { 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x08, 0x03 }, "Erase" },
        { { 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x08, 0x04 }, "Assign" },
        { { 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x08, 0x05 }, "Deassign" },
		...
		{ { 0x00, 0x00, 0x00, 0x09, 0x00, 0x01, 0xff, 0x01 }, "PSID" },
		{ { 0x00, 0x00, 0x00, 0x09, 0x00, 0x03, 0x00, 0x00 }, "Users" },
		...
		{ { 0x00, 0x00, 0x02, 0x01, 0x00, 0x03, 0x00, 0x01 }, "TPerInfo Table" },
		{ { 0x00, 0x00, 0x02, 0x05, 0x00, 0x00, 0x00, 0x01 }, "AdminSP" },
		{ { 0x00, 0x00, 0x02, 0x05, 0x00, 0x00, 0x00, 0x02 }, "LockingSP" },
		{ { 0x00, 0x00, 0x02, 0x05, 0x00, 0x01, 0x00, 0x01 }, "Enterprise LockingSP" },
		{ { 0x00, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x01 }, "Locking Info" },
		{ { 0x00, 0x00, 0x08, 0x02, 0x00, 0x00, 0x00, 0x00 }, "Locking Table" },
		{ { 0x00, 0x00, 0x08, 0x02, 0x00, 0x00, 0x00, 0x01 }, "Locking Global Range" },
        { { 0x00, 0x00, 0x08, 0x03, 0x00, 0x00, 0x00, 0x01 }, "MBR Control" },
        { { 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x08, 0x04 }, "Table Table MBR Table row" },
        { { 0x00, 0x00, 0x08, 0x04, 0x00, 0x00, 0x00, 0x00 }, "MBR Table" },
        { { 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x10, 0x01 }, "Table Table Datastore Table row" },
        { { 0x00, 0x00, 0x10, 0x01, 0x00, 0x00, 0x00, 0x00 }, "Datastore Table" },
    ...
    // band-specific UID?  Assume 15 bits of band.
    uint32_t band = uint32_t(((buf[6] & 0x7F) << 8) | buf[7]) - 1;
    if (band == uint32_t(-1)) return 0;

    // UID table LSB should be 0x01 is the "band" starts with 1, 0x00 if the band starts with 0.
    struct uidlist list2[] =
    {
		{ { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x80, 0x00 }, "BandMaster%u" },
		{ { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x02, 0x01 }, "Admin%u" },
		{ { 0x00, 0x00, 0x00, 0x09, 0x00, 0x01, 0x00, 0x01 }, "Admin%u" },
		{ { 0x00, 0x00, 0x00, 0x09, 0x00, 0x03, 0x00, 0x01 }, "User%u" },
		{ { 0x00, 0x00, 0x00, 0x0B, 0x00, 0x00, 0x02, 0x01 }, "C_PIN_Admin%u" },
        { { 0x00, 0x00, 0x00, 0x0B, 0x00, 0x01, 0x00, 0x01 }, "C_PIN_Admin%u" },
        { { 0x00, 0x00, 0x00, 0x0B, 0x00, 0x03, 0x00, 0x01 }, "C_PIN_User%u" },
		{ { 0x00, 0x00, 0x08, 0x02, 0x00, 0x00, 0x00, 0x00 }, "Locking Range %u" },
		{ { 0x00, 0x00, 0x08, 0x02, 0x00, 0x03, 0x00, 0x01 }, "Locking Range %u" },
		{ { 0x00, 0x00, 0x08, 0x06, 0x00, 0x00, 0x00, 0x00 }, "Band%u_AES_256" },
		{ { 0x00, 0x00, 0x08, 0x05, 0x00, 0x03, 0x00, 0x01 }, "K_AES_128_Range%u_Key" },
		{ { 0x00, 0x00, 0x08, 0x06, 0x00, 0x03, 0x00, 0x01 }, "K_AES_256_Range%u_Key" },
    };
    ...
    {
        if (memcmp(list2[n].UID, buf, sizeof(list2[n].UID) - 1)) continue;
        if ((list2[n].UID[6] & 0x7F) == 0)
        {
            if (list2[n].UID[6] != (buf[6] & 0x80)) continue;
        }
        else
        {
            if (list2[n].UID[6] != buf[6]) continue;
            band &= 0xFF;
        }
        return fprintf(stream, list2[n].name, band + list2[n].UID[7]);
    }
